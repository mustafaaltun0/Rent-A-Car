...
<div class="expenses-page">
  <div class="expenses-hero mb-4">
    <div>
      ...
      <h2 class="mb-2">Isletme Giderleri</h2>
    </div>
  ...
  <div class="alert alert-success">Isletme gideri arsive alindi.</div>
  <?php elseif ($status === 'restored'): ?>
  <div class="alert alert-success">Isletme gideri arsivden geri yuklendi.</div>
  <?php endif; ?>

  <div class="card shadow-sm">
    <div class="card-header"><?= $showArchived ? 'Arsivlenmis Giderler' : 'Gider Listesi' ?></div>
    <div class="card-body border-bottom bg-light-subtle rentals-switchbar">
      <?php if ($showArchived): ?>
      <a href="expenses.php" class="btn btn-outline-dark btn-sm">Normal Listeye Don</a>
      <?php else: ?>
      <a href="expenses.php?show_archived=1" class="btn btn-outline-secondary btn-sm">Arsivdekileri Gor</a>
      <?php endif; ?>
    </div>
    <div class="card-body table-responsive">
      <table class="table table-bordered table-striped align-middle">
        <tr><th>Aciklama</th><th>Tutar</th><th>Tarih</th><th>Islem</th></tr>
        ...
        <tr>
          ...
          <td class="table-actions-cell">
            <div class="action-group">
              <?php if (!$showArchived): ?>
              <button class="action-btn action-warning" type="button" title="Duzenle" aria-label="Duzenle" data-bs-toggle="modal" data-bs-target="#expenseModal" data-mode="edit" data-id="<?= h($expense['id']) ?>" data-title="<?= h($expense['title']) ?>" data-amount="<?= h($expense['amount']) ?>" data-expense_date="<?= h($expense['expense_date']) ?>">
              ...
              <form action="actions/expense_delete.php" method="post" class="d-inline">
                ...
                <button class="action-btn action-danger" type="submit" title="Arsivle" aria-label="Arsivle" data-confirm="Bu gider kaydini arsive almak istediginize emin misiniz?">
              ...
              <form action="actions/expense_restore.php" method="post" class="d-inline">
                <?= auth_csrf_input() ?>
                <input type="hidden" name="id" value="<?= h($expense['id']) ?>">
                <button class="action-btn action-secondary" type="submit" title="Geri Yukle" aria-label="Geri Yukle" data-confirm="Bu gider kaydini arsivden geri yuklemek istiyor musunuz?">
      ...
      <?= pagination_render($expensesPagination, ['item_label' => 'gider kaydi']) ?>
    </div>
  </div>
</div>
    ...
    <div class="modal-content">
      ...
      <form action="actions/expense_save.php" method="post" data-modal-form="expense">
        <div class="modal-body">
          ...
          <div class="mb-3"><label class="form-label">Gider Aciklamasi</label><input name="title" class="form-control" required></div>
